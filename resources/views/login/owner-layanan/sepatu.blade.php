...
<body class="bg-white min-h-screen flex justify-center items-start p-4">
  <div class="w-full max-w-md rounded-md overflow-hidden shadow-sm border border-gray-200">
    ...
    <div class="relative">
      <img alt="Pair of shoes inside a front-loading washing machine drum with water and soap bubbles" class="w-full object-cover md:h-56 h-48" height="224" src="https://storage.googleapis.com/a1aa/image/b32d9c01-f610-4810-bab1-834552053192.jpg" width="600"/>
    ...
    <main class="p-5 space-y-3 bg-white">
      <ul class="space-y-3">
        @forelse ($layanans as $layanan)
          <li>
            <div class="flex justify-between items-center border border-gray-300 rounded-md px-3 py-2">
              <span class="italic font-semibold text-gray-800">{{ $layanan->nama_layanan }}</span>
              <div class="flex items-center space-x-3 text-gray-800 text-sm">
                <span>Rp{{ number_format($layanan->harga, 0, ',', '.') }}</span>
                <a href="{{ route('layanan.edit', $layanan->id) }}" class="text-gray-700 hover:text-gray-900 focus:outline-none" aria-label="Edit {{ $layanan->nama }}">
                  <i class="fas fa-pencil-alt"></i>
                </a>
                <form action="{{ route('layanan.destroy', $layanan->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus layanan ini?')" class="inline">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="text-pink-600 hover:text-pink-700 focus:outline-none" aria-label="Delete {{ $layanan->nama_layanan }}">
        ...
        @empty
          <li class="text-gray-500 italic text-sm text-center">Belum ada layanan tersedia.</li>
        @endforelse
      </ul>

      <div class="flex justify-center mt-6">
        <a href="{{ route('layanan.tambah') }}"
