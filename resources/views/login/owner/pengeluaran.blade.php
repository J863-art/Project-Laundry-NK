...
<body class="bg-white min-h-screen flex justify-center items-start p-4">
  <div class="w-full max-w-md bg-white border border-transparent">
          ...
          <div class="flex justify-between items-start">
            ...
            <div class="text-right flex flex-col justify-between items-end ml-4">
              ...
              <div class="flex space-x-4 mt-4">
                {{-- <a href="{{ route('pengeluaranowner.edit', $item->id) }}" aria-label="Edit {{ $item->judul }}" class="text-gray-600 hover:text-gray-800">
                  <i class="fas fa-pencil-alt text-lg"></i>
                </a> --}}
                {{-- <form action="{{ route('pengeluaranowner.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?')">
                  @csrf @method('DELETE')
    ...
    {{-- <footer class="fixed bottom-4 w-full flex justify-center">
      <a href="{{ route('pengeluaranowner.create') }}">
