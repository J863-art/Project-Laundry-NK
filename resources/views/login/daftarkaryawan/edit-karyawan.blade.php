...
<div class="w-full max-w-xs bg-white shadow-md rounded-md">
    <header class="bg-[#d8396a] flex justify-between items-center px-6 py-4 rounded-t-md">
        <h1 class="text-white font-extrabold text-xl">Edit Karyawan</h1>
    ...
    <form class="p-6 space-y-4"
          action="{{ route('karyawan.update', $user) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        {{-- Password (opsional) --}}
        <div>
            <label for="password" class="block text-sm mb-1">Password baru (opsional)</label>
            <div class="relative">
        ...
        <div>
            ...
            <div class="relative">
                <input id="password_confirmation" name="password_confirmation" type="password" placeholder="konfirm"
                       class="w-full border border-gray-300 rounded-md px-4 py-2 pr-10 placeholder-gray-600
                              focus:outline-none focus:ring-2 focus:ring-[#d8396a]"/>
            ...
            <div class="flex items-center space-x-6">
                ...
                <label class="flex items-center space-x-2 cursor-pointer">
                    <input type="radio" name="gender" value="perempuan"
                           class="form-radio text-[#d8396a]"
                           {{ old('gender', 'laki') == 'perempuan' ? 'checked' : '' }}>
